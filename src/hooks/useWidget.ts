...
export const useWidget = (widgetId?: number): UseWidgetReturn => {
  ...
  const loadWidgets = useCallback(async () => {
    ...
    try {
      const response = await apiClient.widgets.getWidgets();
      if (response.data) {
  ...
  const loadWidget = useCallback(
    async (id: number) => {
      ...
      try {
        const response = await apiClient.widgets.getWidget(id);
        if (response.data) {
  ...
  const createWidget = useCallback(
    async (data: CreateWidgetRequest): Promise<Widget | null> => {
      if (!isAuthenticated) {
      ...
      try {
        const response = await apiClient.widgets.createWidget(data);
        if (response.data) {
  ...
  const updateWidget = useCallback(
    async (id: number, data: UpdateWidgetRequest): Promise<Widget | null> => {
      if (!isAuthenticated) {
        setError("Authentication required");
        return null;
      }
      ...
      try {
        const response = await apiClient.widgets.updateWidget(id, data);
        if (response.data) {
          // Update widgets list
          setWidgets((prev) =>
            prev.map((w) => (w.id === id ? response.data : w)),
          );
  ...
  const deleteWidget = useCallback(
    async (id: number): Promise<boolean> => {
      ...
      try {
        await apiClient.widgets.deleteWidget(id);
  ...
  const toggleWidgetStatus = useCallback(
    async (id: number): Promise<boolean> => {
      ...
      try {
        const response = await apiClient.widgets.toggleWidgetStatus(id);
        if (response.data) {
          // Update widgets list
          setWidgets((prev) =>
            prev.map((w) =>
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to toggle widget status";
        setError(errorMessage);
