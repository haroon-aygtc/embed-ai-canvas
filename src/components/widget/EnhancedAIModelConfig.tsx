...
import {
  ...
  MetricCard,
  FeatureCard,
} from "@/components/ui/EnterpriseCard";
...
export const EnhancedAIModelConfig = ({
  config,
  onConfigChange,
}: EnhancedAIModelConfigProps) => {
  ...
  const [systemPrompt, setSystemPrompt] = useState(
    "You are a helpful customer support assistant for ChatWidget Pro. Be friendly, professional, and provide accurate information about our chat widget platform.",
  );
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      const transformedProviders = response.data.map((apiProvider) => {
        const frontendProvider = transformApiProviderToFrontend(apiProvider);
        return {
          id: frontendProvider.id,
          name: frontendProvider.name,
          icon: getProviderIcon(frontendProvider.id),
          iconColor: getProviderIconColor(frontendProvider.id),
          description: frontendProvider.description,
          status: apiProvider.is_active
            ? ("connected" as const)
      ...
      await loadAllModels(response.data.filter((p) => p.is_active));
    } catch (error) {
      const errorMessage =
  ...
  const loadAllModels = async (apiProviders: ApiProvider[]) => {
    ...
    try {
      for (const apiProvider of apiProviders) {
        ...
        const providerModels = response.data.map((apiModel) => {
          const frontendModel = transformApiModelToFrontend(apiModel);
          if (frontendModel.isActive) {
            activeModelIds.add(frontendModel.id);
          }
          return {
            id: frontendModel.id,
            name: frontendModel.name,
            description: frontendModel.description,
            cost: `${frontendModel.inputCost}/1k`,
            recommended: frontendModel.isDefault,
            contextWindow: `${Math.round(frontendModel.contextWindow / 1000)}K`,
            ...
            isActive: frontendModel.isActive,
            isSaved: frontendModel.isSaved,
  ...
  const handleTestModel = async () => {
    ...
    setTimeout(() => {
      setTestResult(
        "✅ Model test successful! Response time: 850ms. The model is working correctly and ready for production use.",
      );
  ...
  const toggleModelActive = async (modelId: string) => {
    ...
    try {
      ...
      const response = await apiClient.getProviders();
      const apiProvider = response.data.find(
        (p) => p.slug === model.providerId,
      );
      if (!apiProvider) return;

      // Get the API model
      const modelsResponse = await apiClient.getProviderModels(apiProvider.id);
      const apiModel = modelsResponse.data.find((m) => m.model_id === modelId);
      if (!apiModel) return;

      const newActiveState = !model.isActive;
      await apiClient.updateModel(apiModel.id, { is_active: newActiveState });

      // Update local state
      setModels((prev) =>
        prev.map((m) =>
          m.id === modelId ? { ...m, isActive: newActiveState } : m,
        ),
      );
      ...
      console.error("Failed to toggle model active state:", error);
      setError(
  ...
  const toggleAllModels = async (active: boolean) => {
    try {
      const modelIds = models.map((m) => m.id);

      // Find all API models and update them
      const response = await apiClient.getProviders();
      for (const apiProvider of response.data) {
        const modelsResponse = await apiClient.getProviderModels(
          apiProvider.id,
        );
        ...
        if (apiModelIds.length > 0) {
          await apiClient.bulkUpdateModels(apiModelIds, { is_active: active });
        }
      }

      // Update local state
      setModels((prev) => prev.map((m) => ({ ...m, isActive: active })));

      if (active) {
        setActiveModels(new Set(modelIds));
      } else {
        setActiveModels(new Set());
      }
    } catch (error) {
      console.error("Failed to toggle all models:", error);
      setError(
  ...
  const sendChatMessage = async () => {
    if (!chatInput.trim() || !testingModel) return;
    ...
    try {
      ...
      const model = models.find((m) => m.id === testingModel);
      if (!model) {
      ...
      const chatResponse = await apiClient.sendChatCompletion(chatRequest);
      const aiContent =
      ...
      const aiMessage = {
        ...
        content: aiContent,
        timestamp: new Date(),
        responseTime: chatResponse.data.response_time || 0,
        tokens: chatResponse.data.usage?.total_tokens || 0,
      };
        ...
        <TabsContent value="model" className="space-y-6">
            ...
            <div className="relative">
              ...
              >
                {providers.map((provider) => (
                  <Card
                    key={provider.id}
                    className={`flex-shrink-0 w-80 cursor-pointer transition-all duration-200 hover:scale-[1.02] focus:outline-none ${
            ...
            actions={
              <div className="flex items-center space-x-2">
                <div className="flex items-center space-x-2 text-sm">
                  <span className="text-muted-foreground">
                    {activeModels.size} of{" "}
                    {providers.flatMap((p) => p.models).length} active
                  </span>
          ...
          >
            <div className="relative">
              ...
              >
                {isLoadingModels ? (
                  <div className="flex-shrink-0 w-80 p-4 text-center">
                          ...
                          <div className="space-y-2 mb-4">
                            ...
                            <div className="w-full bg-muted rounded-full h-1.5">
                              <div
                                ...
                                style={{
                                  width: `${model.performance.accuracy}%`,
                                }}
                              />
                            </div>
                          </div>
                ...
                <Card className="flex-shrink-0 w-80 border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-primary/50 transition-colors cursor-pointer">
                  <CardContent className="p-4 h-full flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Connect your own AI model or configure additional
                      providers
                    </p>
              ...
              <div className="flex justify-center mt-4 space-x-2">
                {Array.from({ length: Math.ceil((models.length + 1) / 4) }).map(
                  (_, index) => (
                    <div
                      key={index}
              ...
              <Card className="p-4">
                ...
                <div className="text-2xl font-bold text-blue-600">
                  {Math.round(
                    models
                      .filter((m) => m.isActive)
                      .reduce((acc, m) => acc + m.performance.speed * 10, 0) /
                      Math.max(models.filter((m) => m.isActive).length, 1),
                  )}
                  ms
                </div>
                <p className="text-xs text-muted-foreground">
          ...
          >
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  This prompt defines how the AI should behave and respond to
                  users
                </p>
              </div>
          ...
          {!testingModel ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                Go to the Configuration tab and click "Test Model" on any AI
                model to start testing.
              </p>
            ...
            <EnterpriseCard
              ...
              description={`Testing ${models.find((m) => m.id === testingModel)?.name || "AI Model"} - Chat with the model to test its capabilities`}
              icon={MessageSquare}
              variant="elevated"
              actions={
                <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-semibold">
                            {models.find((m) => m.id === testingModel)?.name ||
                              "AI Model"}
                          </h4>
                          <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                            ...
                            <span>
                              {
                                providers.find((p) =>
                                  p.models.some((m) => m.id === testingModel),
                                )?.name
                              }
                            </span>
                          </div>
                    ...
                    >
                      {chatMessages.map((message) => (
                        ...
                        >
                          ...
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center ${message.type === "user" ? "order-1 ml-2" : "order-2 mr-2"}`}
                          >
                            {message.type === "user" ? (
                          ...
                          {chatMessages.filter((m) => m.responseTime).length > 0
                            ? Math.round(
                                chatMessages
                                  ...
                                  .reduce(
                                    (acc, m) => acc + (m.responseTime || 0),
                                    0,
                                  ) /
                                  chatMessages.filter((m) => m.responseTime)
                                    .length,
                              )
                            : 0}
                          ms
              ...
              <div className="space-y-4 pr-2">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              {kb.documents} documents • {kb.size} • Updated{" "}
                              {kb.lastUpdated}
                            </div>
                          </div>
                          <Badge
                ...
                <Card className="border-2 border-dashed border-gray-300 dark:border-gray-600">
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Drag and drop files or click to browse. Supports PDF, TXT,
                      CSV, and more.
                    </p>
